...
@bp.route("/covid19")
def covid19():
    return render_template("test/covid19.html")


@bp.route("/get_covid19_data", methods=("POST", ))
def get_covid19_data():
    df_age = pd.read_csv("pybo/static/csv/나잇대별_확진자.csv")
    df_gender = pd.read_csv("pybo/static/csv/성별_확진자.csv")

    date = request.form.get("date")

    TF = False

    filter_age = df_age["날짜"] == date
    filter_gender = df_gender["날짜"] == date

    list_age = df_age[filter_age].values.tolist()
    list_gender = df_gender[filter_gender].values.tolist()

    if list_age != []:
        TF = True

    age_col = list(df_age.columns)
    gender_col = list(df_gender.columns)

    return {"TF":TF, "age_col": age_col, "gender_col": gender_col, "list_age":list_age, "list_gender":list_gender}



@bp.route("/covid19_map")
def covid19_map():
    return render_template("test/covid19_map.html")
